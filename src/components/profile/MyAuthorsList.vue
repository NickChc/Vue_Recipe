...
<template>
  <div
    class="rounded-lg bg-add-2 p-3 mt-6 w-full sm:w-auto sm:min-w-fit text-primary relative"
  >
    ...
    >
      <ul
        :class="`list-none ${
          showAll ? 'overflow-y-auto max-h-34 scrollbar-small pr-1' : ''
        }`"
      >
        <li v-for="author in authorsToShow" :key="author.id" class="my-2">
          <RouterLink
            :to="`/users/${author.id}`"
